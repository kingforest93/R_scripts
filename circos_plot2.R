...
genebed <- read.table(args[2], sep = "\t", header = TRUE)
genebed <- data.frame(chr = genebed[,1], start = genebed[,3], end = genebed[,4], value = genebed[,5])
tebed <- read.table(args[3], sep = "\t", header = TRUE)
tebed <- data.frame(chr = tebed[,1], start = tebed[,3], end = tebed[,4], value = tebed[,5])
trbed <- read.table(args[4], sep = "\t", header = TRUE)
trbed <- data.frame(chr = trbed[,1], start = trbed[,3], end = trbed[,4], value = trbed[,5])
gcbed <- read.table(args[5], sep = "\t", header = TRUE)
...
}, ylim = c(0, 100), track.height = 0.05, bg.col = NA, bg.border = "black")
...
}, ylim = c(0, 2000), track.height = 0.05, bg.col = NA, bg.border = "black")
