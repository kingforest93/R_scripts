...
if( is.na(args[1]) ) {
	cat("Usage: Rscript /public/agis/fanwei_group/wangsen/bin/heatmap.MWAH_lachesis.R heatmap.txt heatmap.chrom_breaks.txt heatmap.MWAH.pdf [width] [height] [shuffle]\n")
	quit()
}
...
if( ! is.na(args[6]) ) {
	shuffle <- 1 # if 1, randomly permute the heatmap to show the 'pre-Lachesis' dataset.
} else {
	shuffle <- 0
}
...
chrom.borders <- as.numeric( scan( chrom_breaks, quiet=TRUE ) )
...
ggsave( filename=pdf.file, plot=p, device="pdf", width=w, height=h )
