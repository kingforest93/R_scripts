...
if (opt$similarity) {
  gp = ggplot(alignments) +
    ...
    theme(
      ...
      axis.text.y = element_text(size = 3, angle = 15)
    ) +
...
# gp
ggsave(filename = paste0(opt$output_filename, ".pdf"), gp, device="pdf", width = opt$plot_size, height = opt$plot_size, units = "in", dpi = 300, limitsize = F)
