...
if (length(args) != 1) {
    cat("Rscript density.R file.tsv\n")
	quit()
}
...
#draw density
dens.p <- ggplot(dat, aes(x=Identity, color=SuperFamily)) +
...
ggsave(paste(args[1], "Identity.pdf", sep="."), dens.p, device="pdf", width=14, height=8, units="cm")
